...
@interface AESEncryptViewController () <UITextFieldDelegate>

@end

@implementation AESEncryptViewController
...
- (void)viewDidLoad {
    ...
    [self.contentTextField setPlaceholder:@"输入待加密的内容"];
    ...
    [btn addTarget:self action:@selector(encrypt) forControlEvents:UIControlEventTouchUpInside];
}
...
- (void)encrypt {
    ...
    NSData *dataToEncrypt = [self.contentTextField.text dataUsingEncoding:NSUTF8StringEncoding];

    // Encrypting data
    NSData *encryptedData = [AESCrypto AES256EncryptWithKey:key data:dataToEncrypt];
    NSLog(@"Encrypted Data: %@", encryptedData);

    CryptoResultViewController *controller = [[CryptoResultViewController alloc] initWithData: encryptedData];
    controller.modalPresentationStyle = UIModalPresentationOverFullScreen;  // 允许背景透明
    controller.view.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.5];  // 半透明背景
    // 通过模态显示视图控制器
